...
async function report() {
  ...
  await nightmare
    ...
    .wait('div.monthly-container') //等待數秒
    .wait(2000) //等待數秒 
    .click('div.category-item:nth-child(1)') //按下「月報」
    .wait(4000) //等待數秒
    .catch((err) => {
      console.log('ERROR');
      // throw err;
...
async function weekendReport() {
  await nightmare
    ...
    .click('li.el-menu-item:nth-child(2)') //按下「家庭能源報告」
    .wait(1000) //等待數秒 
    // .click('div.el-date-editor--week') //按下「datepicker」
    .goto('https://www.lowcarbon-hems.org.tw/news/weeklyNews?time=2024-05-06', headers) // 进度到当前网址，所以如果想返回，也可以只有.goto()
    // .wait(1000) //等待數秒 
    // .click('tr.el-date-table__row:nth-child(1)') //按下「週報」
    // .wait(2000) //等待數秒
    // .click(
    //   'div.vfc-week:nth-child(1) .vfc-day:nth-child(1) .vfc-span-day:nth-child(1'
    // ) //按下「月報」
    .wait(2000); //等待數秒
}
...
async function reportParseHtml() {
  ...
  //將重要資掀放到陣列中，以便後續儲存
  // 同儕用電比較
  let userCompare = $(html).find('#chartBar');
  let userCompareStatus = await elmStatus(3, '月報-同儕用電比較', userCompare, true);
  // 用電總結
  let summarize = $(html).find('div.rank-part');
  let summarizeStatus = await elmStatus(3, '月報-上月節電排名', summarize);
  // 用電量比較
...
async function monthParseHtml() {
  ...
  // 電器用電佔比
  let proportion = $(html).find('#main');
  let proportionStatus = await elmStatus(3, '週報-電器用電佔比', proportion, true);
  // 節電建議
  let show = $(html).find('div.advice-img p');
...
try {
  asyncArray([
    report, // 月報
    reportParseHtml, // 月報 資料檢查
    weekendReport, // 週報
    monthParseHtml, // 週報 資料檢查
    close,
  ]).then(async function () {
    ...
    await writeFile(`downloads/lowcarbon/${formattedDate}_step3.json`, JSON.stringify(arrLink, null, 4));
